...
enum {
    SE_I2C_STATUS=1, SE_I2C_DATA, SE_INVALID_DATA, SE_SENSOR_ERROR
};
...
check_home(struct ldc1612 *ld, uint32_t raw_data)
{
    ...
    if (!(homing_flags & LH_CAN_TRIGGER))
        return;
    if (check_homing_data_bits(ld, raw_data))
        return;
...
// Report a sample read error to the host
static void
report_sample_error(struct ldc1612 *ld, int error_code, uint8_t oid)
{
...
ldc1612_query(struct ldc1612 *ld, uint8_t oid)
{
    ...
    irq_enable();
    if (ret) {
        report_sample_error(ld, SE_I2C_STATUS, oid);
        return;
    }
