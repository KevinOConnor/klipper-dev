...
def parse_log(logname):
    ...
    for line in f:
        ...
        if parts[0] == 'sample:':
            ...
            continue
        if parts[0] == 'probe_analysis:':
            #probe_analysis: coeffs=(1.0, 2.0, 3.0, 4.0)
            coeffs_str = ' '.join(parts[1:]).strip()
            coeffs = ast.literal_eval(coeffs_str[7:])
            out.append((coeffs, samples))
            samples = []
    f.close()
    return out
...
def plot_cal(data):
    ...
    def formula(z, coeffs):
        z_contact, freq_contact, depress_slope, slope, slope2 = coeffs
        zdiff = z - z_contact
        if zdiff < 0.:
            return freq_contact + zdiff * depress_slope
        return freq_contact + zdiff * slope + zdiff * zdiff * slope2
    lines = []
    for coeffs, samples in data:
        # Generate fit line
        min_z = samples[0][1]
        max_z = samples[-1][1]
        line_zs = list(numpy.arange(min_z, max_z, 0.001))
        line_freqs = list([formula(z, coeffs) for z in line_zs])
        lines.append((coeffs[0], samples, line_zs, line_freqs))
