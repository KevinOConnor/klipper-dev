...
class EddyCalibration:
    def __init__(self, config):
        ...
        gcode.register_mux_command("EDDY_PROBE_CALIBRATE", "CHIP",
                                   cname, self.cmd_EDDY_PROBE_CALIBRATE,
                                   desc=self.cmd_EDDY_PROBE_CALIBRATE_help)
    ...
    def height_to_freq(self, height):
        ...
        if pos == 0 or pos >= len(rev_zpos):
            raise self.printer.command_error("Invalid eddy_probe height")
    ...
    def post_manual_probe(self, kin_pos):
        ...
        gcode = self.printer.lookup_object("gcode")
        gcode.respond_info(
            "eddy_probe: stddev=%.3f in %d queries\n"
    ...
    cmd_EDDY_PROBE_CALIBRATE_help = "Calibrate eddy current probe"
    def cmd_EDDY_PROBE_CALIBRATE(self, gcmd):
...
class EddyEndstopWrapper:
    ...
    def multi_probe_begin(self):
        if not self._calibration.is_calibrated():
            raise self._printer.command_error("Must calibrate eddy_probe first")
